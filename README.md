...
Sol authors or amends ordinary `DISPATCH` packets. A Terra High parent may author exactly one packet only with `PLANNER_ROLE: parent`, `PLANNER_CAPABILITY: bounded_l1_l2_dispatch`, and all fast-path evidence below; it cannot amend a Sol packet or relax the predicate. Missing fields, non-relative write paths, open major decisions, or missing/ineligible fast-path evidence make the packet invalid before Luna. Luna then performs no implementation work and returns `BLOCKED / missing_dispatch / NEXT parent` without naming a planning role.
...
This is a conditional capability of a **Terra High host-model parent**, not a fourth Agent/profile and not a second Sol. The runtime guard accepts `PLANNER_ROLE: parent` with `PLANNER_CAPABILITY: bounded_l1_l2_dispatch` only when every item below is explicit:
