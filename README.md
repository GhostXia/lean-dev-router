...
When the host exposes timestamps, record component-ready and next-stage times. With capacity available, relay or queue the next stage within 60 seconds; report external compile, CI, and network waits separately from controllable handoff delay. During long parent commands, keep completion-event consumption responsive or record why a ready job stayed queued. Terra assignments are ordinary read-only task instructions: `STATUS: DISPATCH` remains reserved for Luna write authorization and must not be reused as an outbound-style Terra envelope.
