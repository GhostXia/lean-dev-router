...
TERRA_ELIGIBILITY_FIELDS = (
    "LEVEL", "OBJECTIVE_FIXED", "SCOPE_ROOTS", "OPEN_MAJOR_DECISIONS",
    "RISK_FLAGS", "EXTERNAL_ACTIONS", "MAX_DISPATCHES", "COMPONENT_COUNT",
    "DEPENDENCY_DEPTH", "REQUIRED_PATHS", "WRITE_BATCH_COUNT",
    "CONTRACT_EXPANDED", "AMBIGUITY",
)
AUDIT_BEGIN_FIELDS = (
...
def validate_terra_dispatch(packet: Mapping[str, Any]) -> list[str]:
    errors = [
        f"{name} must be present for terra_planner"
        for name in TERRA_ELIGIBILITY_FIELDS
    ...
    if _value(packet, "OBJECTIVE_FIXED") is not True:
        errors.append("OBJECTIVE_FIXED must be true")
    if _value(packet, "OPEN_MAJOR_DECISIONS") is not False:
        errors.append("OPEN_MAJOR_DECISIONS must be false")
    if not _none_items(_value(packet, "RISK_FLAGS")):
        errors.append("RISK_FLAGS must be none")
    if not _none_items(_value(packet, "EXTERNAL_ACTIONS")):
        errors.append("EXTERNAL_ACTIONS must be none")
    exact_counts = {
        "MAX_DISPATCHES": {1},
        "COMPONENT_COUNT": {1, 2},
    ...
    for name, allowed in exact_counts.items():
        value = _value(packet, name)
        if isinstance(value, bool) or not isinstance(value, int) or value not in allowed:
            errors.append(f"{name} is outside the terra_planner fast path")
    if _value(packet, "CONTRACT_EXPANDED") is not False:
        errors.append("CONTRACT_EXPANDED must be false")
    if _value(packet, "AMBIGUITY") is not False:
        errors.append("AMBIGUITY must be false")
    roots = _paths(_value(packet, "SCOPE_ROOTS"))
    required = _paths(_value(packet, "REQUIRED_PATHS"), allow_empty=True)
    allowed_paths = _paths(_value(packet, "PATHS_ALLOW"))
    if roots is None:
    ...
    if roots is not None:
        for label, paths in (("PATHS_ALLOW", allowed_paths), ("REQUIRED_PATHS", required)):
...
def validate_dispatch(packet: Mapping[str, Any]) -> list[str]:
    ...
    if planner_role not in {"sol_planner", "terra_planner"}:
        errors.append("PLANNER_ROLE must identify an authorized planner")
    elif planner_role == "terra_planner":
        errors.extend(validate_terra_dispatch(packet))
