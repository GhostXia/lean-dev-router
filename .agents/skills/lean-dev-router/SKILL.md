...
- Before accepting a Luna `PASS`, the current coordinator—Sol for a routed batch or the parent for a direct fast path—independently collects tracked changes with `git diff --name-only --no-renames <baseline> --` and untracked files with `git ls-files --others --exclude-standard`, then verifies every path is covered by `paths_allow`. Record the result as `path: N/A (scope-check)` command evidence; on failure also list each extra path. Do not silently ignore any path class. / 接受 Luna 的 `PASS` 前，当前协调者——路由批次中的 Sol 或直接快路径中的父会话——必须独立使用 `git diff --name-only --no-renames <baseline> --` 收集 tracked 改动，并使用 `git ls-files --others --exclude-standard` 收集 untracked 文件，再确认每个路径均被 `paths_allow` 覆盖。将结果记录为 `path: N/A (scope-check)` 命令证据；失败时还要列出每个额外路径。不得静默忽略任何路径类别。
...
- Before terminal whole-task `PASS`, the coordinator must verify the integration worktree is clean, collect tracked paths with `git diff --name-only --no-renames <integration_baseline> <combined_commit> --` and untracked files with `git ls-files --others --exclude-standard`, and confirm all paths are covered by `integration_paths_allow`. Record this as `path: N/A (scope-check)`, then record the combined commit, integration order, and acceptance commands as `path: N/A (integration-check)`. / 返回整体任务最终 `PASS` 前，协调者必须确认集成 worktree 干净，使用 `git diff --name-only --no-renames <integration_baseline> <combined_commit> --` 收集 tracked 路径，并使用 `git ls-files --others --exclude-standard` 收集 untracked 文件，再确认全部路径都包含在 `integration_paths_allow` 中。将结果记录为 `path: N/A (scope-check)`，再以 `path: N/A (integration-check)` 记录组合提交、集成顺序和验收命令。
