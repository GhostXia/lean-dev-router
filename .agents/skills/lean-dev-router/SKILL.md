...
- Every repository claim must bind to a concrete relative path and include a short diff summary or command result. Use `path: N/A (planning-only)` only for planning with no repository artifact, `path: N/A (scope-check)` for a repository-wide allow-list result, and `path: N/A (integration-check)` for commands run against the combined integration commit; never invent a path. / 每个仓库结论都必须绑定具体相对路径，并附简短 diff 摘要或命令结果。只有纯规划且没有仓库文件时可使用 `path: N/A (planning-only)`，仓库级 allow-list 结果使用 `path: N/A (scope-check)`，针对组合后集成提交运行的命令使用 `path: N/A (integration-check)`；不得伪造路径。
...
- When two or more write batches form one deliverable, a component `PASS` closes only that batch and never implies whole-task success. Sol must define shared contracts, dependency order, an integration worktree or clean checkout, and `integration_acceptance` before dispatch. / 当两个或更多写入批次共同组成一个交付物时，组件级 `PASS` 只关闭对应批次，绝不代表整体任务成功。Sol 必须在派发前定义共享契约、依赖顺序、集成 worktree 或干净 checkout，以及 `integration_acceptance`。
- Integrate accepted batches into the same clean state in dependency order. Prefer incremental convergence: after each dependent batch or independent wave, run the narrowest useful cross-batch checks; after all batches are combined, run the complete integration acceptance against the exact combined commit. / 将已接受批次按依赖顺序合入同一个干净状态。优先增量收敛：每个依赖批次或独立波次合入后运行最小必要的跨批检查；全部组合后，必须针对确切的组合提交运行完整集成验收。
- Sol may return terminal whole-task `PASS` only with `path: N/A (integration-check)` evidence naming the combined commit, integration order, and command results. If the plan requires Terra validation, a Terra must inspect that combined state; separate component audits are not a substitute. / Sol 只有在 `path: N/A (integration-check)` 证据中列出组合提交、集成顺序和命令结果后，才可返回整体任务的最终 `PASS`。如果计划要求 Terra 验证，Terra 必须审查组合后的状态；各组件分别通过审计不能替代集成审计。
- On integration failure, stop terminal success and identify the earliest failing merge or wave. Route an obvious bounded compatibility repair to Luna, unclear cross-component causality to Terra, an in-scope contract or decomposition change to Sol, and any user-owned objective, compatibility, or product trade-off through the human decision gate. / 集成失败时不得宣布最终成功，并定位最早失败的合入或波次。明确且边界清晰的兼容修复交给 Luna，跨组件原因不明确时交给 Terra，需要在范围内调整契约或拆分时交给 Sol；涉及用户专属目标、兼容性或产品取舍时进入用户决策门。
